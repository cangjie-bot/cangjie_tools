    ...
    const std::string &file, const std::string &contents, Position pos, bool onlyParse, const std::string &name)
{
    ...
    if (fileKind == CangjieFileKind::IN_PROJECT_NOT_IN_SOURCE) {
        ...
        const auto &commonSpecificPaths = GetInstance()->GetCommonSpecificModuleSrcPaths(modulePath);
        std::string parentPath;
        for (auto &commonSpecificPath : commonSpecificPaths) {
            if (IsUnderPath(commonSpecificPath, absName)) {
                parentPath = Normalize(commonSpecificPath);
                break;
            }
        }
        if (parentPath.empty()) {
        ...
        auto pkgInfo = 
            std::make_unique<PkgInfo>(dirPath, moduleInfo.modulePath, moduleInfo.moduleName, callback, PkgType::SPECIFIC);
...
std::pair<CangjieFileKind, std::string> CompilerCangjieProject::GetCangjieFileKind(const std::string &filePath, bool isPkg) const
{
    ...
    for (const auto &item : moduleManager->moduleInfoMap) {
        bool isCommonSpecificModule = item.second.isCommonSpecificModule;
        if (isCommonSpecificModule) {
            const auto &commonSpecificPaths = GetInstance()->GetCommonSpecificModuleSrcPaths(item.first);
...
void CompilerCangjieProject::InitOneModule(const ModuleInfo &moduleInfo)
{
    ...
    const auto &pkgType = moduleInfo.isCommonSpecificModule ? PkgType::COMMON : PkgType::NORMAL;
    ...
    for (const auto &path : moduleInfo.commonSpecificPaths.second) {
        // root specific package path
        const auto &specificPath = Normalize(path);
        if (pkgInfoMap.find(rootPackageName) == pkgInfoMap.end()) {
            continue;
        }
        auto derivativePackage = 
            std::make_unique<PkgInfo>(specificPath,
                moduleInfo.modulePath, moduleInfo.moduleName, callback, PkgType::SPECIFIC);
        this->pathToFullPkgName[specificPath] = rootPackageName;
        ...
        for (auto &packagePath: GetAllDirsUnderCurrentPath(specificPath)) {
            packagePath = Normalize(packagePath);
            std::string pkgName = GetRealPkgNameFromPath(
                GetPkgNameFromRelativePath(GetRelativePath(specificPath, packagePath) |IdenticalFunc));
            std::string fullPackageName = SpliceFullPkgName(moduleInfo.moduleName, pkgName);
            if (pkgInfoMap.find(fullPackageName) == pkgInfoMap.end()) {
                continue;
            }
...
std::vector<std::string> CompilerCangjieProject::GetSourceSetNamesByPackage(const std::string &packageName)
{
    ...
    const auto &moduleInfo = moduleManager->moduleInfoMap[moduleName];
    const auto &moduleSpecificPaths = moduleInfo.commonSpecificPaths.second;
    sourceSetNames.push_back("common");
    if (pkgInfo->derivativePackages.empty() || moduleSpecificPaths.empty()) {
        return sourceSetNames;
    }
    for (size_t i = 0; i < pkgInfo->derivativePackages.size() 
        && i < moduleSpecificPaths.size() && i < moduleInfo.sourceSetNames.size() - 1; i++) {
        const auto &derivativePackagePath = pkgInfo->derivativePackages[i]->packagePath;
        const auto &derivativeModulePath = moduleSpecificPaths[i];
