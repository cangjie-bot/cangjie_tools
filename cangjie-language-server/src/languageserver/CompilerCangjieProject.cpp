...
void CompilerCangjieProject::IncrementCompile(const std::string &filePath, const std::string &contents, bool isDelete)
{
    ...
    if (changed) {
        ...
        // UpdatePackageStatusInCI(fullPkgName, ChangeState::CHANGED);
    }
    ...
    CompilerInstance::SrcCodeChangeState state)
{
    ...
    if (CIForParse && CIForParse->pkgNameForPath == pkgName) {
        std::lock_guard lock(CIForParse->fileStatusLock);
        if (CIForParse->fileStatus.find(file) != CIForParse->fileStatus.end() &&
            CIForParse->fileStatus[file] != CompilerInstance::SrcCodeChangeState::ADDED) {
            // just update the existed file status, ignore add and then change case
            CIForParse->fileStatus[file] = CompilerInstance::SrcCodeChangeState::CHANGED;
        }
    }
}

void CompilerCangjieProject::UpdatePackageStatusInCI(const std::string& pkgName, ChangeState state)
{
    if (CIForParse) {
        std::lock_guard lock(CIForParse->pkgStatusLock);
        if (CIForParse->allUpstreamPkgStatus.find(pkgName) != CIForParse->allUpstreamPkgStatus.end()) {
            // just update the existed file status, ignore add and then change case
            CIForParse->allUpstreamPkgStatus[pkgName] = ChangeState::CHANGED;
        }
    }
}
} // namespace ark
