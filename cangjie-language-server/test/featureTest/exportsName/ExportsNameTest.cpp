...
bool LspExportsNameTest(TestParam param)
{
    ...
    bool showErr = CheckExportsNameResult(expLines, result, info);
    if (!showErr) {
...
class ExportsNameTest : public testing::TestWithParam<struct TestParam> {
protected:
    void SetUp()
    {
        SetUpConfig("exportsName");
    }
};

INSTANTIATE_TEST_SUITE_P(ExportsName, ExportsNameTest, testing::ValuesIn(GetTestCaseList("exportsName")));

TEST_P(ExportsNameTest, ExportsNameTestCase)
{
    TestParam param = GetParam();
    ASSERT_TRUE(LspExportsNameTest(param));
}