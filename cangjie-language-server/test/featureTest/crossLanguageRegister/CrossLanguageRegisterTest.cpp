...
bool LspCrossLanguageRegisterTest(TestParam param)
{
    ...
    bool showErr = CheckCrossLanguageRegisterResult(expLines, result, info);
    if (!showErr) {
...
class CrossLanguageRegisterTest : public testing::TestWithParam<struct TestParam> {
protected:
    void SetUp()
    {
        SetUpConfig("crossLanguageRegister");
    }
};

INSTANTIATE_TEST_SUITE_P(CrossLanguageRegister, CrossLanguageRegisterTest, testing::ValuesIn(GetTestCaseList("crossLanguageRegister")));

TEST_P(CrossLanguageRegisterTest, CrossLanguageRegisterTestCase)
{
    TestParam param = GetParam();
    ASSERT_TRUE(LspCrossLanguageRegisterTest(param));
}