...
TEST(UtilsTest, DeleteCharForPosition_AtLineBreak_CRLF) {
    std::string text = "A\r\nB";
    bool result = DeleteCharForPosition(text, 1, 2); // Try to delete between CR and LF
    EXPECT_TRUE(result);
...
// Mock FileUtil functions for GetAllFilePathUnderCurrentPath testing
namespace FileUtil {
    // Mock implementation for testing
    std::vector<std::string> GetAllFilesUnderCurrentPath(
        const std::string& path,
        const std::string& extension,
        bool shouldSkipTestFiles,
        bool shouldSkipRegularFiles) {

        if (path == "empty_dir") {
            return {};
        }
        if (path == "single_file_dir") {
            return {"file1.cj"};
        }
        if (path == "multiple_files_dir") {
            return {"file1.cj", "file2.cj", "test_file.cj"};
        }
        if (path == "subdir1") {
            return {"subfile1.cj"};
        }
