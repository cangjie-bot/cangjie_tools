...
static void MyCustomFunction4(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
    ...
    int sum = a + b;
    if (sum == 0) {
        traits::Result<std::optional<int>>::call(ctx, std::nullopt);
    } else {
        traits::Result<std::optional<int>>::call(ctx, sum);
    }
}

TEST(FunctionTest, callTest003)
{
    const std::string dbPath = "mydatabase.db";
    sqlite3* db = openDatabase5(dbPath);
    if (!db) {
