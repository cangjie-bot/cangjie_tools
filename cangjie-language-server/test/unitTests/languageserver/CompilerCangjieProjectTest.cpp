...
TEST_F(CompilerCangjieProjectTest, GetCIForDotCompleteTest) {
    ...
    Position pos = {0, 1, 25}; // Position before dot
    
    // Test getting compiler instance for dot completion
    auto ci = CompilerCangjieProject::GetInstance()->GetCIForDotComplete(filePath, pos, contents);
...
TEST_F(CompilerCangjieProjectTest, GetCangjieFileKindTest) {
    std::string filePath = "test_workspace/src/test_pkg/test_file.cj";

    // Test Cangjie file kind detection
    auto result = CompilerCangjieProject::GetInstance()->GetCangjieFileKind(filePath);
    // Verify file kind classification
}

TEST_F(CompilerCangjieProjectTest, GetIncTopologySortTest) {
    std::string pkgName = "test_pkg";

    // Test incremental topological sorting
    auto sorted = CompilerCangjieProject::GetInstance()->GetIncTopologySort(pkgName);
    // Verify sorting result
}

TEST_F(CompilerCangjieProjectTest, EmitDiagsOfFileTest) {
    std::string filePath = "test_workspace/src/test_pkg/test_file.cj";

    // Test diagnostics emission for file
    CompilerCangjieProject::GetInstance()->EmitDiagsOfFile(filePath);
    // Verify diagnostics processing
}

TEST_F(CompilerCangjieProjectTest, CheckPackageNameByAbsNameTest) {
    // Create minimal file structure for testing
    File testFile;
    testFile.filePath = "test_workspace/src/test_pkg/test_file.cj";

    std::string fullPackageName = "test_pkg";

    // Test package name validation by absolute file name
    CompilerCangjieProject::GetInstance()->CheckPackageNameByAbsName(testFile, fullPackageName);
