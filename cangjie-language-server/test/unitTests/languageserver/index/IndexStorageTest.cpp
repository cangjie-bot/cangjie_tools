...
protected:
    std::string testRoot = "./test_idx_root";
...
TEST_F(IndexStorageTest, GetShardPath_PathConversion) {
    ...
    CacheManager cm("/tmp/test_cache");
...
TEST_F(IndexStorageTest, CacheManager_InitDir_Failure) {
    // Test directory creation failure by using an invalid path or read-only area
    // This targets 'if (ret == -1) { return; }' in InitDir
    CacheManager cm("/dev/null/invalid");
    cm.InitDir();
