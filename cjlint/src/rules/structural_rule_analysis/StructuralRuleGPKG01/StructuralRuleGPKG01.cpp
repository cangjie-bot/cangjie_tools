...
std::string ProcessPrefixIncludeOrgName(const std::vector<std::string>& prefixPaths, bool hasDoubleColon)
{
    auto prefix = Utils::JoinStrings(prefixPaths, ".");
    if (hasDoubleColon) {
...
void StructuralRuleGPKG01::CheckImportItemName(const Cangjie::AST::ImportSpec& importSpec)
{
    ...
    if (ic.kind == AST::ImportKind::IMPORT_ALL) {
        auto prefix = ProcessPrefixIncludeOrgName(ic.prefixPaths, ic.hasDoubleColon);
        ...
        if (!ic.prefixPaths.empty()) {
            prefix = ProcessPrefixIncludeOrgName(ic.prefixPaths, ic.hasDoubleColon) + ".";
        }
